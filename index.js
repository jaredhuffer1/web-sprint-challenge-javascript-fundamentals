...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(array){
    const lowCase = array.map(i => i.animal_name.toLowerCase())
    return (lowCase);
   }
  ...
  */
  ...
  function USApop(array){
    return(array.reduce((callback, item) => callback + item.population, 0))
  }
...
function CuboidMaker(cubeSize){
  this.length = cubeSize.length;
  this.width = cubeSize.width;
  this.height = cubeSize.height;
}
...
const cubMaker = new CuboidMaker ({
  length: 4,
   width: 5,
    height: 5
  })
...
constructor(cuTwo){
  this.height = cuTwo.height;
  this.length = cuTwo.length;
  this.width = cuTwo.width;
}
surfaceArea(){
  return 2 * ((this.length * this.width) + (this.length * this.height) + (this.width * this.height))
}
